...
uint32_t turnOffDeviceDelayTime = 172800;
uint8_t flagSendWarnningBeforeTurnOff = 0;

int32_t resetTimeCnt = 86400; // one day
...
void RTCAlarm_IRQHandler(void)
{
	...
	TIME_FromSec(&localTime,rtcTimeSec + sysTimeZone);	
	...
	if(ACC_IS_ON) 
	{	
		turnOffDeviceDelayTime = 172800;
		PWR_EN_PIN_SET;
	}
	
	if(turnOffDeviceDelayTime)
	{
		turnOffDeviceDelayTime--;
		if(turnOffDeviceDelayTime == 300)
		{
			flagSendWarnningBeforeTurnOff = 1;
		}
	}
	else
	{
