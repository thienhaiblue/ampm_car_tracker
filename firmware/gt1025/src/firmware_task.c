...
void FIRMWARE_Task(uint8_t *buf)
{
//	uint32_t i;
//	if(fDataLength)
//	{
//		GPRS_ProtoRecv.dataPt = buf;
...
//				fwBuf[i] = 0;
//		}
//		CfgProcessData(&GPRS_ProtoRecv,&GPRS_ProtoSend,buf,GPRS_parserPacket.lenMax - 4,0);
		...
		case INITIAL:
			if(fwSwitchServer == 0)	
			{
					...
					else
					{
							...
							if(fwServerIp == NULL && (SysTick_Get() - fwCheckDns >= 10000))
							{
			...
			fwConn = tcp_connect(fwServerIp, uip_htons(50000),FIRMWARE_Callback,NULL);
			fwTick = SysTick_Get();
			if(fwConn == NULL)
			{
			...
			memcpy((char  *)fInfo.id,(char  *)sysCfg.imei,18);
