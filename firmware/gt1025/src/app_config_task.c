...
uint32_t  DbgCfgPrintf(const uint8_t *format, ...)
{
	...
	for(i = 0;i < len; i++)
	{
		if(logSource != NO_LOG && logSource != GPS_LOG)
		{
			if(RINGBUF_Put(&configTaskRingBuf, buffer[i])){
				RINGBUF_Get(&configTaskRingBuf, &c);
...
uint8_t CfgProcessData(CFG_PROTOCOL_TYPE *cfgProtoRecv,CFG_PROTOCOL_TYPE *cfgProtoSend,uint8_t *cfgSendDataBuff,uint32_t maxPacketSize,uint8_t logSendEnable)
{
	...
	switch(cfgProtoRecv->opcode)
	{
		...
		case 0x0F:
			...
			{
				DB_SaveSpeed_Pause();
				if(f_open(&file,(char *)pt.dataPt, FA_READ|FA_OPEN_EXISTING) != FR_OK)
				{
					pt.packetNo = 0xffffffff;
				}
				else
				{
						recordFileSize = f_size(&file);
					if(recordFileSize == 0){
						pt.packetNo = 0xffffffff;
					}
					else if(recordFileSize >= (sizeof(SPEED_RECORD)*3600*2))
					{
						f_unlink((char *)pt.dataPt);
					}
				}
				cfgProtoSend->start = 0xCA;
			...
			else
			{
				...
				if(f_size(&file) == 0 || f_lseek(&file,pt.packetNo) != FR_OK)
				{
					pt.packetNo = 0xffffffff;
					cfgProtoSend->length = 4;
					f_close(&file);
				...
				else if(f_read(&file,&cfgSendDataBuff[4],maxPacketSize, &j) != FR_OK)
				{
					pt.packetNo = 0xffffffff;
					cfgProtoSend->length = 4;
					f_close(&file);
		...
		case 0xFB:						// format disk
			...
			if(pt.packetNo == 0xA5A5A5A5)
			{
				BuzzerSetStatus(&buzzer1Ctrl,100, 3000,BUZZER_TURN_ON, 100);//300ms on/off , 5 times
				watchdogEnable[WTD_MAIN_LOOP] = 0;
				if(f_mkfs(0, 0, 1) == FR_OK)
				{
					BuzzerSetStatus(&buzzer1Ctrl,100, 100,BUZZER_TURN_ON, 3);//300ms on/off , 5 times
					SysTick_DelayMs(1000);
				}
				else 
				{
					BuzzerSetStatus(&buzzer1Ctrl,100, 100,BUZZER_TURN_ON, 1);//300ms on/off , 5 times
					SysTick_DelayMs(1000);
				}
				DB_SaveAll();
				NVIC_SystemReset();
...
// 								DelayMs(1);
		...
		case 0xFD:					// factory reset
			...
			if(pt.packetNo == 0xA5A5A5A5)
			{
				strcpy((char *)buf, (char *)sysCfg.imei);
				memset((void*)&sysCfg, 0xFF, sizeof(CONFIG_POOL));
				strcpy((char *)sysCfg.imei ,(char *)buf);
				CFG_Save();
