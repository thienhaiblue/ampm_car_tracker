...
void CFG_Load(void)
{
	...
	for(i = 0; i < CONFIG_SIZE_PHONE_NUMBER_MAX;i++)
		if(sysCfg.userPhone[i][0] == 0xff)
		{
			strcpy((char *)sysCfg.userPhone[i], DEFAULT_USER_PHONE_NUMBER);
			saveFlag = 1;
		}
	...
	for(i = 0; i < CONFIG_SIZE_RFID_USER_MAX;i++)
		if(sysCfg.rfidPass[i] == NULL)
		{
			strcpy((char *)sysCfg.rfidPass[i] ,DEFAULT_RFID_TAG_ID);
			saveFlag = 1;
		}

	if(sysCfg.deviceTurnOffDelay == 0xffffffff)
	{
		sysCfg.deviceTurnOffDelay = 24*3600;
	}
