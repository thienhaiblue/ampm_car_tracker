...
uint8_t VoiceCall_Task(void)
{
	...
	switch (voiceCall_TaskPhase)
	{
		...
		case WAIT_ANSWER_CALL:
			...
			{
				//ModemCmdTask_SendCmd(NULL,NULL,100, modemOk, modemError, 3000, 0, "AT^SIND=\"dtmf\",1\r");
				if (atCmdCallParser.cancelCall)
				{
					AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#CALL# HANG_UP_CALL\r\n");
					voiceCall_TaskPhase = HANG_UP_CALL;
		...
		case HANG_UP_CALL:
			COMM_Putc(0x1A);
			if(callSentATH == 0)
				ModemCmdTask_SendCmd(NULL,NULL,100, modemOk, "NO CARRIER", 2000, 2, "AT+CHLD=0\r");
			else
		...
		case WAIT_HANG_UP_CALL:
			...
			{
				if (atCmdCallParser.hangUpCallRetry++ < 3)
				{
					AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#CALL# HANG_UP_CALL\r\n");
					voiceCall_TaskPhase = HANG_UP_CALL;
				}
...
uint32_t GetCallerNumber(uint16_t cnt,uint8_t c)
{
	...
	else 
	{
		if(c == '"' && cnt >= 2)
		{
			memcpy(atCmdCallParser.incomingCallPhoneNo,incomingCallPhoneNo,sizeof(incomingCallPhoneNo));
