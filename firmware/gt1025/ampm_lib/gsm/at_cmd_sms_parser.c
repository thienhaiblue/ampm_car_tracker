...
extern uint16_t CMD_CfgParse(char *cmdBuff, uint8_t pwdCheck,uint8_t sourceCmd);

uint32_t GetServiceCenterNumber(uint16_t cnt,uint8_t c);
uint32_t GetCSQ(uint16_t cnt,uint8_t c);
...
uint8_t SmsCheckMessage_IsEmpty(void)
{
	...
	while(smsMsg != NULL)
	{
		if(smsMsg->flag == SMS_NEW_MSG && smsMsg->msg != NULL && smsMsg->phone != NULL)
		{
			return 0;
		}
...
uint8_t Sms_Task(void)
{
	...
	switch(smsPhase)
	{
		case SMS_IDLE:
			if(GsmTask_GetPhase() == MODEM_SYS_COVERAGE_OK)
			{
				if(smsNewMessageFlag)
				{
					...
					AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_START_RECV0\r\n");
					smsPhase = SMS_START_RECV_CHECK_CSQ; 
					break;
				}
				if(smsCancelAll)
				{
			...
			break;
		case SMS_START_RECV_CHECK_CSQ:
			if (CheckTimeout(&tSmsTaskTimeOut) == SYSTICK_TIMEOUT)
			{
				if(ModemCmdTask_IsIdle())
				{
					ModemCmdTask_SendCmd(GetCSQ,"+CSQ:",100, modemOk, modemError, 2000, 1, "AT+CSQ\r") ;
					AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_START_RECV0\r\n");
					smsPhase = SMS_START_RECV0;
				}
			}
		case SMS_START_RECV0:
			if (CheckTimeout(&tSmsTaskTimeOut) == SYSTICK_TIMEOUT)
			{
				if(ModemCmdTask_IsIdle())
				{
					ModemCmdTask_SendCmd(NULL,NULL,100, modemOk, modemError, 2000, 1, "AT+CBC\r") ;
					AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_START_RECV1\r\n");
					smsPhase = SMS_START_RECV1;
				}
			}
		break;
		case SMS_START_RECV1:
			if (ModemCmdTask_GetPhase() == MODEM_CMD_OK)
			{
				ModemCmdTask_SendCmd(NULL,NULL,100, modemOk, modemError, 1000, 1, "AT+CMGF=1\r") ;
				AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_START_RECV2\r\n");
				smsPhase = SMS_START_RECV2;
			}
		...
		break;
		case SMS_START_RECV2:
			if (ModemCmdTask_GetPhase() == MODEM_CMD_OK)
			{
				ModemCmdTask_SendCmd(NULL,NULL,500, modemOk, modemError, 3000, 1, "AT+CMGL=\"ALL\"\r") ;
				AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_START_RECV3\r\n");
				InitTimeout(&tSmsTaskTimeOut, SYSTICK_TIME_SEC(3));
				smsPhase = SMS_START_RECV3;
			}
			else if(ModemCmdTask_GetPhase() == MODEM_CMD_ERROR)
			{
				AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_IDLE\r\n");
			...
			if (CheckTimeout(&tSmsTaskTimeOut) == SYSTICK_TIMEOUT)
			{
				if(ModemCmdTask_IsIdle())
				{
					if(RINGBUF_GetFill(&smsUnreadRingBuff) >=2)
					{
							...
							AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_READ_MSG\r\n");
							smsPhase = SMS_READ_MSG;
					}
					else
					{
						smsNewMessageFlag = 0;
		...
		case  SMS_READ_MSG:
			if (CheckTimeout(&tSmsTaskTimeOut) == SYSTICK_TIMEOUT)
			{
				...
				ModemCmdTask_SendCmd(NULL,NULL,500, modemOk, modemError, 3000, 1, "AT+CMGR=%d\r",smsIndex) ;
				flagSmsReadFinish = 0;
				AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_READ_AND_DELETE\r\n");
				smsPhase = SMS_READ_AND_DELETE;
			}
		break;
		case SMS_READ_AND_DELETE:
			if(ModemCmdTask_IsIdle())
			{
				ModemCmdTask_SendCmd(NULL,NULL,500, modemOk, modemError, 3000, 1, "AT+CMGD=%d\r",smsIndex) ;
				AMPM_GSM_LIB_DBG("\r\nAMPM_LIB:#SMS# SMS_READ_PROCESS\r\n");
				smsPhase = SMS_READ_PROCESS;
			}	
		break;
		case SMS_READ_PROCESS:
			if(ModemCmdTask_IsIdle())
			{
				if(flagSmsReadFinish)
				{
					...
					smsDataBuf[i] = 0;
					smsDataBuf[i + 1] = 0;
					CMD_CfgParse((char *)smsDataBuf, 1,CMD_FROM_SMS);
					CFG_Save();
				}
				smsPhase = SMS_START_RECV3;
				InitTimeout(&tSmsTaskTimeOut, SYSTICK_TIME_MS(500));
		...
		case SMS_SENT:
			...
			break;
		case SMS_SEND_FAIL:
			if (CheckTimeout(&tSmsTaskTimeOut) == SYSTICK_TIMEOUT && ModemCmdTask_IsIdle())
			{
				smsMsg->flag = SMS_MSG_FAIL;
				smsTaskFailCnt++;
				if(smsTaskFailCnt >= SMS_MAX_RETRY) 
